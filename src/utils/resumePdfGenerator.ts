...
class ResumePDFGenerator {
  ...
  private addText(
    ...
    for (const line of lines) {
      if (centerAlign) {
        ...
        const xPosition = (this.settings.pageWidth - textWidth) / 2;
  ...
  private addContactInfo(contact: any): void {
    ...
    // Contact details in a compact format - center aligned
    ...
    if (contactDetails.length > 0) {
      this.addText(contactDetails.join(' | '), this.settings.fontSize.small, 'normal', this.settings.colors.secondary, 0, undefined, true);
      this.addSpace(8);
    }
  }
  ...
  private addWorkExperience(experiences: any[]): void {
    ...
    experiences.forEach((exp, index) => {
      ...
      // Dates and location - left aligned even though gray
